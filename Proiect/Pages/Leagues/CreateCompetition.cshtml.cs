...
namespace Proiect.Pages.Leagues
{
    public class CreateCompetitionModel : PageModel
    {
        ...
        public void OnPost()
        {
			...
			if (info.id.Length == 0 || info.name.Length == 0 || info.numberOfTeams.Length == 0  || info.marketValue.Length == 0)
			{
			...
			try
			{
				...
				{
					connection1.Open();
					String sql1 = "SET IDENTITY_INSERT [competition] ON INSERT INTO competition " + "(id, name, numberOfTeams, marketValue) VALUES " + "(@id, @name, @numberOfTeams, @marketValue)";
			...
			info.numberOfTeams = "";
			info.marketValue = "";
			succesMessage = "Competition created succesfully";
